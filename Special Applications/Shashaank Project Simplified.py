...
unresolved_by_esep = set([m for m in fundamental_unlabelled if m.all_esep_unlabelled in metagraph.latent_free_esep_patterns])
print(f"Number of {n}-vis node unlabelled mDAGs not resolved by e-sep: {len(unresolved_by_esep)}")
unresolved_by_esep_plus = set([m for m in fundamental_unlabelled if m.all_esep_plus_unlabelled in metagraph.latent_free_esep_plus_patterns])
...
still_remaining_after_supports_on_four_events = set([m for m in still_remaining_after_supports_on_two_events if m.no_infeasible_binary_supports_beyond_dsep_up_to(4)])
